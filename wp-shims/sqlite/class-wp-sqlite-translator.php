...
class WP_SQLite_Translator {
	...
	/**
	 * D1-DIVERGENCE: set when the database rejects BEGIN (D1 has no interactive
	 * transactions); begin/commit/rollback become level-tracking no-ops.
	 *
	 * @var bool
	 */
	private $d1_transactions_disabled = false;
	...
	public function __construct( $pdo = null ) {
		...
		// D1-DIVERGENCE: pdo_d1 has no sqliteCreateFunction() and D1 cannot call
		// back into PHP. MySQL functions emulated via PHP UDFs (DATE_FORMAT, FIELD,
		// REGEXP, ...) are unavailable on D1; queries using them will error.
		...
		// D1-DIVERGENCE: production D1 permits only a small PRAGMA subset
		// (foreign_keys and table introspection among them); encoding/journal_mode
		// are rejected. miniflare (local dev) is more permissive than production -
		// these guards are analytic, not just empirical. Failures are non-fatal.
		try {
			$statement = $this->pdo->query( 'PRAGMA foreign_keys' );
			// phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
			if ( $statement && $statement->fetchColumn( 0 ) == '0' ) {
				$this->pdo->query( 'PRAGMA foreign_keys = ON' );
			}
		} catch ( PDOException $e ) {
			error_log( 'wp-shims/db.php notice: PRAGMA foreign_keys unavailable: ' . $e->getMessage() );
		}
		try {
			$this->pdo->query( 'PRAGMA encoding="UTF-8";' );
		} catch ( PDOException $e ) {
			// D1 is UTF-8 only; losing this PRAGMA is harmless.
		}
		// D1-DIVERGENCE: journal_mode is managed by D1 itself; never issue it.
	}
	...
	public function get_sqlite_version(): string {
		// D1-DIVERGENCE: D1 blocks SQLITE_VERSION() ('not authorized to use
		// function') - observed live in miniflare, so it applies locally AND in
		// production. Fall back to a nominal version (D1 ships SQLite 3.4x;
		// callers only use this for display/feature gates).
		try {
	...
	private function update_data_type_cache( $table, $column_or_index, $mysql_data_type ) {
		$this->execute_sqlite_query(
			'INSERT INTO ' . self::DATA_TYPES_CACHE_TABLE . ' (`table`, `column_or_index`, `mysql_type`)
				...
				ON CONFLICT(`table`, `column_or_index`) DO UPDATE SET `mysql_type` = excluded.`mysql_type` -- D1-DIVERGENCE: pdo_d1 forbids named-placeholder reuse (native-positional driver); excluded.* avoids it
			',
			array(
				':table'    => $table,
	...
	public function begin_transaction() {
		// D1-DIVERGENCE: D1 has no interactive transactions (BEGIN/COMMIT/ROLLBACK
		// are rejected; batch() is its primitive). On the first failure we disable
		// transactions for the request and degrade gracefully: WordPress core runs
		// without transactions. A notice is logged once. (ADR-0025; pdo_d1 ADR-0022.)
		if ( $this->d1_transactions_disabled ) {
		...
		try {
			...
			$this->d1_transactions_disabled = true;
			error_log( 'wp-shims/db.php notice: transactions unavailable on this database; continuing without them (' . $e->getMessage() . ')' );
			++$this->transaction_level;
			return true;
		} finally {
			if ( $success ) {
	...
	public function commit() {
		...
		--$this->transaction_level;
		if ( $this->d1_transactions_disabled ) {
			return true; // D1-DIVERGENCE: no-op (see begin_transaction).
		}
		if ( 0 === $this->transaction_level ) {
